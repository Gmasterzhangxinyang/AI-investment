...
> 当前运行口径以 [`docs/CURRENT_SYSTEM.md`](docs/CURRENT_SYSTEM.md) 和 `configs/strategy_params.json` 为准。历史 `docs/superpowers/plans/`、`specs/` 记录的是实施过程，不代表当前默认配置。
...
- 交易信号由确定性规则生成。
- AI 基于受控证据独立做研究分析，可以评价规则结论，但不能生成、修改或覆盖系统正式信号。
...
| ETF | 当前默认 `trend_pullback_v2`：同时判断中期趋势与短期入场，过滤 MA20 下行、周 MACD 走弱和巨量长阳假反转；仍可切回 `legacy_v1`。 |
| TL | 对 30 年国债期货 TL 做日频状态诊断：不做交易、关注交易、模型触发建仓候选。 |
| 可转债 | 当前启用“`legacy_v1` 基础排名 + `dynamic_v2` 四项短期辅助”；辅助层不改基础分、资格和排名，也不会强行凑满 Top10。 |
| 标的档案 | 按代码或名称查看 ETF、TL、可转债的单标的指标、状态、风险说明和入库数据。 |
| 数据状态 | 查看源文件、SQLite、dashboard、数据质量和最近运行状态。 |
| 系统状态 | 查看刷新结果、数据覆盖和需处理问题；技术详情按需展开。 |
...
大模型只在独立研究层使用，规则层仍负责正式信号：

- 日报和问答的策略结果来自 `dashboard.json`、SQLite 和配置参数。
- 配置的 LLM provider 不可用、额度不足或超时时，系统回退到本地确定性解释。
- 前端没有独立计算策略，所有页面展示后端生成的证据包。

投研问答使用三档运行路径：明确查数、单标的和多 ETF 当前状态由规则快速回答；对象明确的解释类问题使用快速模型；跨资产综合分析、复杂排序解释或问题不明确时才启动受控 ReAct 主 Agent。主 Agent 可以自行选择只读白名单工具并反思证据是否足够，但不能直接执行 SQL、访问任意文件/网络、改策略或下单。详细边界见 `docs/CHAT_AGENT_RUNTIME.md`。

刷新采用原子发布：所有新结果先写入独立 staging 目录，通过数据质检和 SQLite 入库后，最后切换 `dashboard.json`。失败时不会覆盖上一版成功结果，页面会显示失败环节、影响和处理建议。
...
当前默认策略是 `trend_pullback_v2`，根据信号日指标和持仓状态判断：

- 持仓中：只检查平仓提示。
- 未持仓或已平仓：分别输出中期趋势和短期入场状态。
- 密切观察：MA5 已在 MA10 上方，且日 MACD 绿柱缩短或转红；同时提示周 MACD 和 MA20 斜率。
- 中期确认：价格与 MA5 站上 MA20、MA20 走平或向上、周/日 MACD 共同确认。
- 短期入场：中期确认后再等待突破确认或缩量回踩承接；过热巨量长阳不追。
- 平仓侧重点：跌破 MA10 或 MA5，并伴随配置要求的放量。

`legacy_v1` 仍作为可切换插件保留，其 MA5/MA10、MACD、量能规则不会混入 v2 的中期/短期状态。
...
当前基础决策层是 `legacy_v1`，负责硬排除、基础分、资格和最终排名；默认启用的 `dynamic_v2` 读取正股涨跌、转债涨跌、前一日转股溢价率和当日溢价率变化，只生成短期联动辅助状态，不改变基础排名。可转债不强行凑满 Top10，输出分层为：
...
| 文档 | 内容 |
| --- | --- |
| `docs/CURRENT_SYSTEM.md` | 当前启用策略、AI 架构、数据范围、刷新和边界的唯一事实清单。 |
...
| `docs/CHAT_AGENT_RUNTIME.md` | 问答三档运行路径、白名单工具、数据范围和AI边界。 |
| `docs/RELEASE_NOTES_2026-07-15.md` | 原子刷新、问答Agent和性能优化版本说明。 |
| `docs/RELEASE_NOTES_2026-07-16.md` | 当前策略、AI Agent 和全量文档一致性更新。 |
| `docs/RELEASE_NOTES_2026-07-17.md` | AI研究层与规则层分离、连续追问和来源标签修复。 |
